...
ggplot(worldsadness)+
  ...
  labs(y= NULL,
       title= "Self-reported loneliness among older adults",
        subtitle = "Share of survey respondents who report feeling lonely at least some of the time. For all \n countries estimates correspond to population ages 65+, except in the following cases: US \n(ages 72+); UK (ages 65-74); and Finland (ages 75+).",
  ...
  theme(
    plot.title = element_text(size=32, family = "Playfair Display", color = "#555555"),
    plot.subtitle = element_text(size=18, family = "Arimo", color = "#555555", lineheight = 0.5, margin= margin(0,0,20,0)),
    plot.caption = element_text(size = 15, family = "Arimo", color = "#555555", lineheight = 0.5,  margin= margin(10,0,0,0)))+
  #now text position
  ...
  geom_text(aes(label = paste(Sadness, Year, sep= "% in ")), vjust = 0.7, hjust=-0.1, colour = "#555555", size= 6, family= "Arimo")+
  #change the grid
...
#Open data set
worldsadness <- read_csv("remake2-self-reported-loneliness-older-adults.csv")  %>%
  select(-Code)  %>%
  rename( Country = Entity)
...
worldsadness$Country <- factor(worldsadness$Country, 
                               levels=c("Denmark","Switzerland", "Sweden",
                                        ...
                                        "Spain", "Belgium","France", "Austria",
                                        "Italy","Israel", "Greece"))
worldsadness$Age <- factor(worldsadness$Age, 
...
ggplot(worldsadness)+
  ...
  #now texts characteristics
  theme(
    plot.title = element_text(size=17, family = "Playfair Display", color = "black"),
    plot.subtitle = element_text(size=9, family = "Arimo", color = "#555555", lineheight = 0.9, margin= margin(0,0,10,0)),
    plot.caption = element_text(size = 7, family = "Arimo", color = "#555555", lineheight = 0.5, margin= margin(10,0,0,0)))+
  #now text position
  theme(plot.title.position = "plot")+
  theme(plot.subtitle = element_text(hjust = 0))+
  theme(plot.caption =  element_text(hjust = 0))+
  theme(plot.caption.position = "plot")+
  #add label at the end of the bar
  geom_text(aes(label = paste(Sadness, "%", sep = "")), vjust = 0.7, hjust=-0.1, colour = "#555555", size= 3, family= "Arimo")+
    #ticks del eje y bien puestos
  scale_y_continuous(limits = c(0,70), labels = c("","","","30%","", "","60%"), breaks = c(0,10,20,30,40,50,60))+  
  #both axis text characteristics modified
  theme(axis.text.x = element_text(size = 10, family = "Arimo",
                                   color = "#555555"))+
  theme(axis.text.y = element_text(size = 9, family = "Arimo", face = "bold", color = "#555555", hjust=1, vjust= 0))
```
...
#The data
worldsadness <- read_csv("map-self-reported-loneliness-older-adults.csv")  %>%
  select(-Code)  %>%
  rename( Country = Entity)
...
mapdata<- Europe %>% left_join(worldsadness, by = c("admin" = "Country"))
...
#scale color and legend
   scale_fill_gradient(low="yellow", high ="red", limits = c(20,70), name=NULL,
                      labels = c("30%","40%", "50%","60%"), breaks = c(30,40,50,60),
...
#now texts characteristics
  theme(
    plot.title = element_text(size=19, family = "Playfair Display", color = "black", lineheight = 0.9),
    plot.subtitle = element_text(size = 9, family = "Lato", color = "#555555", lineheight = 0.8, margin= margin(10,0,0,0)),
    plot.caption = element_text(size = 8, family = "Lato", color = "#555555", lineheight = 0.8, margin= margin(10,0,0,0)))+
  #now text position
  theme(plot.title.position = "plot")+
...
mylabels <- paste(ifelse(is.na(mapdata$name), "", mapdata$name), "<br/>",
                  ifelse(is.na(mapdata$Sadness), "", paste(mapdata$Sadness,"%")), "<br/>",
                   ifelse(is.na( mapdata$Year), "", paste("Data collection:", mapdata$Year)), "<br/>",
                  ifelse(is.na(mapdata$Age), "", paste("Respondents age:", mapdata$Age)))%>%
  lapply(htmltools::HTML)

#title and reference
htmltitle <- "<h5> Percentage of elders \n feeling lonely </h5>"
references <- "<h5>Source: Our World in Data based on Sundström et al. (2009), Savikko et al (2005), ONS (2019) and CIGNA (2018) </h5>"
